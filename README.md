...
- **[crewAIInc/crewAI](https://github.com/crewAIInc/crewAI)** (58k+ stars) — unified six scattered, drifting copies of "which model maps to which context window" into a single source of truth, fixing two real bugs in the process: a current OpenAI model silently falling back to an 8,192-token default through the LiteLLM path, and a Bedrock/Claude model getting a stale 200k-token window instead of its real 1M. Every one of ~155 existing entries was extracted programmatically and verified byte-for-byte before any file was touched; a real bug in my own refactor (a validation-scope narrowing that silently broke an existing test) was caught only by actually running the test suite, not by reasoning about the design.
- **[opendp/tumult-analytics](https://github.com/opendp/tumult-analytics)** — `Session.delete_view()` could delete private tables, not just views, silently, with no distinguishing check — deleting the last one crashed with an unhelpful internal error instead of a clear one at the actual mistake. Added tracking for which tables are genuine views and a clear rejection otherwise.
- **[isair/jarvis](https://github.com/isair/jarvis)** (1.7k+ stars) — surfaced Whisper's low-confidence transcription rejections as a structured event (confidence + transcript) accessible to downstream code, laying the groundwork for spoken/visual feedback on misheard speech. Applied identically across both speech-recognition backends (faster-whisper and MLX), with a dedicated test confirming a hard hallucination rejection and a low-confidence rejection stay independent signals.
